...
\chapter{Propriétés générales des systèmes auto-gravitants}
	...
	\section{Amas Globulaires}
		...
		\subsection{Simulation numérique des amas globulaire}
...
%	Le raisonnement est le même pour la limite en énergie.
	...
	\section{Les Galaxies}%{Que peut-on dire des autres ...!}
		...
		\subsection{Historique}
			...
			Au début des années 1930, \bsc{Zwicky}, travaillant sur l'amas de galaxies Coma, se rendit compte que la masse
			gravitationnelle ne correspondait pas à la masse lumineuse (voir~\cite{1933AcHPh...6..110Z}): si l'on suppose, comme le fait
			\bsc{Zwicky}, que l'amas de la Coma est dans un état d'équilibre, le théorème du Viriel impose alors que sa masse, son rayon
		...
		\subsection{Profils de matière noire}
			...
			Il ressort de cette étude qu'il n'existe pas de profil universel, comme les travaux de la fin du \romannumeral 20
			\textsuperscript{e}~siècle tentaient de le trouver (voir par exemple~\cite{NFW+97}). Chaque halo de matière noire pour chaque
			...
			de cœur-halo, ce sont les seules à posséder un cœur. Les galaxies spirales et elliptiques possèdent généralement un profil
			montrant un cups à la place du cœur (un profil de type cupside) ajusté correctement par un profil de Vaucouleurs.
			...
			Dés 1948, Gérard de Vaucouleurs remarque que de nombreuses galaxies elliptiques possédaient une
		...
		\subsection{Scénario de formation}
			...
			\begin{itemize}
				\item il prévoit une structure de type cupside pour les halos de matière noire alors que les observations
					montrent que les galaxies naines possèdent un cœur et un halo et qu'elles sont très nombreuse;
