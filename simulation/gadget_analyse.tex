...
\label{Chap::Simu::Analysis}
	...
	\section{Le programme de simulation}
		...
		\subsection{\og\Sm\fg et pas de temps}
			...
			% \begin{align*}
				...
				% \end{cases}
			% \end{align*}
			Le \sm est schéma d'intégration d'ordre 2. Il a la particularité d'introduire un décalage entre l'évaluation des positions est
			...
			\begin{align*}
				...
				\end{cases}
			\end{align*}
			avec $\vec{a}$ l'accélération subit par la particule. La figure~\ref{Fig::Num::LeapFrog} illustre le fonctionnement de ce
			schéma.
			\begin{figure}[htpb]
			...
			L'algorithme utilisé par Gadget est basé sur ce schéma \sm.
			...
			pas de temps $\frac{\Delta t}{2}$ afin de les synchroniser avec positions. Ces vitesses sont ensuite utilisé pour incrémenter
			...
			\begin{figure}[htpb]
				...
				\end{tikzpicture}
				\caption{Principe de fonctionnement du prédicteur-correcteur de \textsc{gadget-2} sur un pas de temps.\label{Fig::Num::GadSchem}}
			\end{figure}
	...
	\section{Observables des simulations}  %\label{Verif_gene}}
		...
		\subsection{Masse et densité}
			...
			pouvons voir que l'accord entre les courbes théorique et calculées est toujours inférieur à $20\%$, sauf au centre et au bord
			du système. Ces deux régions sont en général trop pauvre en particule pour que nous puissions y avoir une précision correcte
			avec notre méthode de calcul.

		\subsection{Énergie et potentiel}

			La partie la plus complexe de l'obtention des observables est le calcul de l'énergie. Nous allons calculer le potentiel avec
			la méthode décrite dans la section~\ref{Sec::KdTree}.
			% Deux
		...
		% de masse et comme masse la masse totale des particules du cube.
			La figure~\ref{potentiel_5000} montre le potentiel calculé avec un angle d'ouverture de $0,5$, et sans lissage, superposé au
			...
			\begin{figure}[htbp]
				\centering \includegraphics{verif_potentiel_new_error.pdf}
		...
		\subsection{Anisotropie des vitesses et forme de l'amas}
			...
			\mbox{$\lambda_1 \geq \lambda_2 \geq \lambda_3$} de la
			matrice d'inertie:
			\begin{align*}
		...
		\subsection{Température cinétique}
			...
			la température calculée par l'équation~\ref{Eq::Temp::Approx} et la théorie. Nous pouvons voir ici que l'erreur relative est
