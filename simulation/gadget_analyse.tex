...
\label{Chap::Simu::Analysis}
	...
	\section{Le programme de simulation}
		...
		\subsection{Calcul du potentiel: $\mathrm{k}^\mathrm{d}$-Tree}
			...
			est le travail de D. Heggie avec presque 300 000 particules (voir sa présentation au trimestre Gravasco) en utilisant la
			version GPU de NBODY6. Dans la limite fluide, nous aurons parfois besoin d'un grand nombre de particules pour représenter la
			fonction de distribution dans l'espace des phases. Nous avons donc besoin d'une approche nous permettant d'accélérer les
			...
			Cet algorithme, nommée \og treecode\og, a été crée par~\cite{1986Natur.324..446B}.
			% Un tel algorithme a été appliqué en astrophysique pour la première fois par \cite{1986Natur.324..446B}.
			Les particules sont
			organisées dans un arbre modulable appelé $\mathrm{k}^\mathrm{d}$-Tree. En dimension $d=3$, et avec une division dichotomique
			...
			En pratique nous nous arrêterons lorsqu'un cube contiendra au plus $N_\mathrm{max}=15$ particules. Les carrées et les points
		...
		\subsection{Lissage de la force}
			...
			Le code \textsc{Gadget-2} utilise une approximation plus sophistiqué consistant à approximer les particules par des nuages de
			taille epsilon et dont le profile suit une fonction \verb|spline|.
	...
	\section{Observables des simulations}  %\label{Verif_gene}}
		...
		\subsection{Masse et densité}
			...
			\begin{figure}[h!]
				...
				\centering \includegraphics{verif_densite_new.pdf}
		...
		% c'est le centre de masse des particules qu'il  conti
			...
			\begin{figure}[h!]
				...
				\centering \includegraphics{verif_potentiel_new.pdf}
		...
		\subsection{Température cinétique}
			...
			\begin{figure}[h]
				\centering \includegraphics{verif_dispersion.pdf}
				\caption{toto}
			\end{figure}
