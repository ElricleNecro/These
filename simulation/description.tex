...
\section{Seconde étude}\label{Sec::2ndStudy}
	...
	réservoir, excepté pour certaines simulations. L'intersection des deux systèmes (bain et \textsc{sag}) n'étant pas vide, le nombre de particules du bain contenu dans le
	...
	\begin{figure}[htbp]
		\centering \rotatebox{90}{%
			\includegraphics[scale=0.8]{graphe/post-collapse_v2.pdf}%
		}
...
\section{Focalisation sur les simulations intéressantes}
	\subsection{Instabilité d'orbites radiales}
		...
		\end{figure}
		Au fur et à mesure que l'accrétion se poursuit, l'anisotropie du \textsc{sag} évolue jusqu'à atteindre un maximum puis diminue. Dans le même
		...
		peupler. C'est bien ce qu'indique l'augmentation progressive du paramètre d'anisotropie $\beta$.
		% lors que l'anisotropie atteint un minimum.
		En effectuant un zoom autour du
		maximum de l'anisotropie (voir la figure~\ref{Fig::Simu::ROI::RAA::Cm34Zoom}), nous observons une diminution significative de $a_2$,
		signe de l'arrivée de l'instabilité vers $t'=11.5$, tandis que le maximum de l'anisotropie est atteint dans l'intervalle $t'\in\left[11; 12\right]$.
		% alors que le maximum de l'anisotropie est atteint vers $t'=12$.
		...
		changement de température du bain n'influe pas de manière significative sur son déclenchement: toutes les simulations de cette catégorie déclenchent
		l'instabilité dans l'intervalle $t'\in\left[11; 12\right]$.
		% vers $t'=12$.
		\begin{figure}[htbp]
		...
		l'instabilité. Il est aisé de remarquer que l'instabilité se produit toujours au même moment, sauf pour
		% les simulations $C_{3.4}$ (figure~\ref{Fig::Simu::ROI::RAA::C34::Zoom}) et
		la simulation $B_{3,1}$ (figure~\ref{Fig::Simu::ROI::RAA::B31::Zoom}).
		%
		...
		a très peu d'effet sur le déclenchement de l'instabilité d'orbite radiale.

		% La première simulation à avoir un comportement différent est la simulation $C_{3,4}$.
		La simulation $C_{3,4}$ et la simulation $C^m_{3,4}$
		...
		leurs évolutions n'est pas complètement similaire.
		La décroissance de $a_2$ et l'évolution de l'anisotropie semblent indiquer que l'instabilité se produit au même moment. Mais $a_1$ montre une croissance
		plus importante. La simulation n'a pas été assez loin pour conclure, en effet, compte tenu du temps de calcul, elle a duré deux fois moins de temps que la simulation $C^m_{3,4}$ ($t'_{C_{3,4}} =
		15$ au lieu de $t'_{C^m_{3,4}} = 23$).
		% La simulation n'est pas allée assez loin pour confirmer la
		...
		que l'instabilité d'orbite radiale se déclenche vers $t'=18$ (voir la figure~\ref{Fig::Simu::ROI::B31}).
		\begin{figure}[htbp]
			\begin{subfigure}{0.50\linewidth}
		...
		radiale se produit. Le maximum de l'anisotropie n'a pas encore été atteint.
	...
	\subsection{Évolution de la pente avec l'âge (effets de relaxation)}
		...
		\begin{figure}[htbp]
			...
			\end{subfigure}\hfill
			\begin{subfigure}{0.45\linewidth}
				\centering \includegraphics{{N-100000.0_Rc-66.6666666666_sc-0.001_evo_anisov2}.pdf}
			...
			% \end{subfigure}
			\caption{Simulation de paramètres $N=100000$, $R_c=66.6$, $\sigma_c=0.001$.\label{Fig::Relax::Data}}
		\end{figure}
