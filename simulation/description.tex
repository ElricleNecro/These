...
\section{Description des conditions initiales}
	...
	L'effet d'un thermostat sur le \textsc{sag} ne peut a priori se faire qu'aux travers des collisions~\footnote{par collisions, nous entendons des
...
\section{Étude préliminaire}
	...
	température du bain, la taille de la boîte et le nombre de particules utilisées dans chacun des deux systèmes. La taille de la boîte
	...
	Ni la densité du \textsc{sag} ni les autres paramètres n'ont jamais montré de véritable évolution lors de ces simulations.
...
\section{Seconde étude}
	...
	\input{simulation/result_array.tex}
	...
	\begin{enumerate}
		...
		\item les simulations \accretionmoyen{} entrent dans une catégorie intermédiaire suggérant que les caractéristiques du bain, comme sa
			densité ou sa température, sont des paramètres important dans le déclenchement de l'instabilité d'orbite radiale.
...
\section{Focalisation sur les simulations intéressantes}
	\subsection{Instabilité d'orbite radiale}
		...
		\begin{figure}[htbp]
			...
			\end{subfigure}
			\caption{Zoom sur le moment de l'instabilité pour les rapports d'axes $a_1$ et $a_2$ et de l'anisotropie.\label{Fig::Simu::ROI::RAA::All}}
		\end{figure}
		...
		la densité du bain sont les mêmes. Nous pouvons alors déduire que la dispersion de vitesse du bain, dans l'intervalle de valeur testé,
		...
		ont été construites pour être le plus similaire possible. Pour construire la simulation $C^m_{3,4}$, nous avons multiplié la taille du
		cube par $1,5$ afin de vérifier si l'effet n'est pas dû aux conditions périodiques. Pour avoir la simulation la plus
		similaire possible, nous avons aussi multiplié le nombre de particule du bain par ${1,5}^3$ pour conserver sa densité.
		% En pratique,
		...
		moment, mais l'anisotropie, après être remontée un peu, recommence à diminuer. La simulation n'est pas allé assez loin pour confirmer la
		présence d'un minimum dans cette zone, en effet, elle a duré deux fois moins de temps que la simulation $C^m_{3,4}$ ($t_{C_{3,4}} =
		8T_d^{t=0}$ au lieu de $t_{C^m_{3,4}} = 16T_d^{t=0}$).
		...
		La dernière simulation à étudier est la simulation $C_{6,1}$. Pour commencer, cette simulation est bien plus évoluée dynamiquement que
		les précédentes. Le rayon $R_{10}$ reste constant tout du long de l'évolution, $R_{50}$ commence à croître vers $t' = 45$. Par contre,
		$R_{90}$ croît tout au long de la simulation, mais cette croissance s'accélère à partir de $t' = 40$ (voir la
		figure~\ref{Fig::Simu::ROI::Radius::B31}). Le rapport d'axe $a_2$ commence à décroître à $t'=40$, signe que l'instabilité d'orbite
		...
		% qui ce produise.
		Même si nous ne l'avons pas observé complètement, il semble bien là aussi que ce soit l'instabilité d'orbite radiale qui soit entrain
		...
		le déclenchement de l'instabilité était paramétré par la densité du système accrèté mais pas par sa température.
	...
	\subsection{Évolution de la pente avec l'âge (effet de relaxation)}
		...
		la durée de la simulation de $15.9T_d^H$ à $47.7T_d^H$. La figure~\ref{Fig::Relax::Data} rassemble les graphiques montrant l'évolution
		...
		simulation, tout en gardant constant tous les paramètres de sorte que les nouvelles simulations soit strictement équivalentes. Les
		différentes graphiques de la figure~\ref{Fig::Relax::Comp} montrent l'évolution de ces nouvelles simulations (les lignes
		...
		\end{figure}
		Nous pouvons voir que plus le nombre de particules augmentent, moins l'effondrement est prononcé.

		% Il ne serait donc pas un effet dû au bain. 
		L'évolution de la densité du \textsc{sag} ne semble donc pas lié à la présence du bain.
		Pour en être complètement sûr, nous allons regarder l'évolution du \textsc{sag} isolé.
		Cette impression est confirmé par l'étude de l'évolution du \textsc{sag} isolé (simulation sans bain).
		La
		figure~\ref{Fig::Relax::CompH} montre l'évolution d'une sphère de Hénon comprenant $3\ 10^4$ particules
		 et d'une sphère de Hénon de $3\ 10^5$ particules. La conclusion apparaît immédiatement: le
		...
		particules du Hénon (en conservant sa masse totale identique), l'évolution de la densité est radicalement changée et ne montre plus
		de changement sur l'intervalle considéré.
		L'effet observé n'est donc pas un effet du bain, il s'agit très certainement d'un effet de la relaxation à deux corps.
		...
		\begin{figure}[htbp]
			...
			\caption{Comparaison de notre simulation avec deux sphères de Hénon isolées composées d'un nombre de particules différent.
				...
				celle d'une sphère de Hénon de $3\ 10^5$ particules.
