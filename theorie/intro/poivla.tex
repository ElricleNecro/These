...
\end{align*}
pour une large classe de densités suffisamment régulières (celles permettant à~\refeq{eq1} de converger), la force dérive donc du potentiel~\refeq{eq2} et
nous avons:
\begin{align}
...
entre champs scalaires. Ceci est possible en remarquant que l'équation~\refeq{eq2} est une convolution:
\begin{align}
...
\end{align*}
Nous allons donc appliquer le laplacien à l'équation~\refeq{poiconvlap} qui s'écrit alors:
\begin{align*}
...
points dans l'espace des phases. En explicitant la divergence de l'équation~\refeq{continu}, il vient:
\begin{align*}
...
\end{align*}
Sous toutes ces hypothèses, la version intégrée~\refeq{liouint} de l'équation de Liouville s'écrit donc:
\begin{align*}
...
\end{align*}
L'équation~\refeq{boltzman} est appelée équation de Boltzmann.
