...
\begin{align}
	T_{d}\propto\left(  G\overline{\rho}\right)  ^{-1/2}\propto T_{cr} \label{def:T-dyn_tcr}
\end{align}

Nous pouvons faire les choses plus précisément mais il faut se donner un potentiel ou une répartition de masse.
Nous pouvons alors écrire l'équation du mouvement d'une particule test à l'intérieur du système. Plusieurs temps caractéristiques apparaissent alors:
\begin{itemize}
...
nous privilégions ici l'extraction d'un ordre de grandeur basé sur un raisonnement de Chandrasekhar.
...
$\delta v^{2}$ s'obtient en combinant~\ref{deltavrel} et~\ref{dnrel} et en sommant sur toutes les valeurs possibles de $p$, soit:
\begin{align*}
...
\end{align*}
La moyenne permettant de calculer le temps de relaxation à 2 corps peut s'envisager sur une période, l'expression~\ref{TREL} s'écrit donc:
\begin{align*}
...
tableau~\ref{Tab::TempsCarac::OG} donne un ordre de grandeurs de ces temps pour différents objets. %résume bien l'affaire
\begin{table}[htbp]
